...
@section('breadcrumb')
    <x-breadcrumb :items="[
        ...
        [
            'label' => __('Application List'),
            'url' => '/public/view_import_permit',
...
@section('content')
    ...
    <div class="apy-wrapper">
        ...
        <div class="apy-hero">
            ...
            <div>
                ...
                <h3 class="apy-hero-title" data-en="Complete Your Payment" data-bm="Selesaikan Pembayaran Anda">Complete
                    Your Payment</h3>
                <p class="apy-hero-sub"
                    data-en="This order covers the permit item(s) below. Review the details, choose a payment method, then proceed."
                ...
                <div class="apy-parties" id="apyParties">
                    <div class="apy-party-card">
                        <div class="apy-party-header">
                            <div class="apy-party-avatar">{{ strtoupper(substr($application->importer->fullname, 0, 1)) }}
                            </div>
                            <div>
                        ...
                        <div class="apy-party-row"><i class="bi bi-telephone"></i>
                            {{ $application->importer->phone_number }}</div>
                        <div class="apy-party-row"><i class="bi bi-envelope"></i>
                            {{ $application->importer->email ?? authUser()['user']->email }}</div>
                    ...
                    <div class="apy-party-card">
                        <div class="apy-party-header">
                            <div class="apy-party-avatar">{{ strtoupper(substr($application->exporter->name, 0, 1)) }}
                            </div>
                            <div>
                        ...
                        <div class="apy-party-row"><i class="bi bi-telephone"></i> {{ $application->exporter->phone_no }}
                        </div>
                        <div class="apy-party-row"><i class="bi bi-envelope"></i>
                            {{ $application->exporter->email ?? '—' }}</div>
                        <div class="apy-party-row"><i class="bi bi-geo-alt"></i> {{ $application->exporter->address }},
                            {{ $application->exporter->countryInfo->name }}</div>
                    </div>
                </div>
            </div>
        @endif
        ...
        <div class="apy-layout">
            ...
            <div class="apy-left-col">
                        ...
                        @foreach ($permits as $index => $permit)
                            <div class="apy-order-row">
                                ...
                                <div class="apy-order-row-info">
                                    <div class="apy-order-row-name">{{ $permit->consignment_detail['item_name'] ?? '—' }}
                                    </div>
                ...
                {{-- ========== 2. CATEGORY SUMMARY (Consignment only) ========== --}}
                @if ($application->application_type == 'Consignment Certificate')
                    @php
                        $prices = json_decode($application->prices_total, true) ?? [];
                    @endphp
                    @if (!empty($prices))
                        <div class="apy-card">
                            ...
                            <div class="apy-order-list">
                                @foreach ($prices as $item)
                                    <div class="apy-order-row">
                                        <div class="apy-order-row-icon"><i class="bi bi-tag"></i></div>
                                        <div class="apy-order-row-info">
                                            <div class="apy-order-row-name">
                                                {{ $item['category_name'] ?? 'Uncategorized' }}</div>
                                            <div class="apy-order-row-meta">
                                                <span>{{ number_format($item['quantity'] ?? 0) }} kg</span>
                                            </div>
                                        </div>
                                        <div class="apy-order-row-fee">RM {{ number_format($item['price'] ?? 0, 2) }}
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                            {{-- @php
                                $totalPrice = array_sum(array_column($prices, 'price'));
                            @endphp
                            <div class="apy-total-row"
                                style="border-top:1px solid var(--gray-3); padding-top:0.75rem; margin-top:0.5rem;">
                                <span class="fw-bold" data-en="Total Price" data-bm="Jumlah Harga">Total Price</span>
                                <span class="fw-bold">RM {{ number_format($totalPrice, 2) }}</span>
                            </div> --}}
                        </div>
                    @endif
                @endif
                ...
                <div class="apy-card">
                    ...
                    <p class="apy-card-hint"
                        ...
                        BayuPay is selected by default — tap the <i class="bi bi-info-circle"></i> icon on either option
                        for step-by-step instructions.
                    </p>
                    <div class="apy-payment-methods" id="apyPaymentMethods">
                        @foreach ($paymentMethod as $item)
                            <div class="apy-pm-option {{ $loop->first ? 'is-selected' : '' }}"
                                data-method="{{ $item->name }}">
                                <label class="apy-pm-option-label">
                                    <input type="radio" name="apyPaymentMethod" value="{{ $item->name }}"
                                        class="apy-pm-radio" {{ $loop->first ? 'checked' : '' }}>
                                    <img src="{{ $item->pic }}" alt="{{ $item->name }}" class="apy-pm-logo">
                                    <span class="apy-pm-name">{{ $item->name }}</span>
                                </label>
                                <button type="button" class="apy-pm-info-btn" data-method-info="{{ $item->name }}"
                                    title="How to pay with {{ $item->name }}">
            ...
            <div class="apy-summary-col">
                <div class="apy-summary-card">
                    ...
                    @elseif($application->application_type == 'Consignment Certificate')
                        ...
                        {{-- Display category-wise price breakdown (like Import Permit's per-item lines) --}}
                        <div class="apy-summary-lines" id="apySummaryLines">
                            @forelse($prices as $item)
                                <div class="apy-summary-line">
                                    <span>{{ $item['category_name'] ?? 'Uncategorized' }}
                                        ({{ number_format($item['quantity'] ?? 0) }} kg)
                                    </span>
                                    <span>RM {{ number_format($item['price'] ?? 0, 2) }}</span>
                                </div>
                            @empty
                                <div class="apy-summary-line text-muted">
                        ...
                        {{-- Optionally, also list the consignment items (permits) --}}
                        @if ($application->consignment_permits && $application->consignment_permits->count())
                            <div class="apy-items-list mt-3">
                                <h6 class="mb-2" data-en="Consignment Items" data-bm="Barangan Konsainan">Consignment
                                    Items</h6>
                                <ul class="list-unstyled">
                                    @foreach ($application->consignment_permits as $permit)
                                        <li class="py-1 border-bottom">
                                            <span class="fw-semibold">{{ $permit->permit_number ?? '—' }}</span>
                                            – {{ $permit->consignment_detail['item_name'] ?? 'Item' }}
                                            ({{ $permit->consignment_detail['quantity'] ?? 0 }}
                                            {{ $permit->consignment_detail['measure'] ?? 'kg' }})
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                        ...
                        {{-- Show total at the bottom (optional if not already shown elsewhere) --}}
                        <div class="apy-total-line mt-3">
                            <span class="fw-bold" data-en="Total Price" data-bm="Jumlah Harga">Total Price</span>
                            <span class="fw-bold">RM {{ number_format($totalPrice, 2) }}</span>
                        </div>
                    @endif
                    ...
                    <div class="apy-totals">
                        @if ($application->application_type == 'Consignment Certificate')
                            <div class="apy-total-row">
                                <span data-en="Category in order" data-bm="Kategori dalam pesanan">Category in
                                    order</span>
                                <span id="kategoriCount">
                                    @if ($application->application_type == 'Consignment Certificate')
                                        {{ count(json_decode($application->prices_total, true) ?? []) }}
                                    @endif
                                </span>
                            </div>
                        @endif
        ...
        <div class="text-center mt-3">
            <button type="button" class="btn btn-outline-secondary" id="returnToApplication"
                data-app-id="{{ $application->application_id }}" data-app-type="{{ $application->application_type }}">
                <i class="bi bi-arrow-left"></i> <span data-en="Return to Application"
                    data-bm="Kembali ke Permohonan">Return to Application</span>
            </button>
        </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content apy-howto-modal-content">
                ...
                <div class="modal-footer border-top">
                    <button type="button" class="apy-btn-secondary" data-bs-dismiss="modal" data-en="Close"
            ...
            <div class="modal-content" style="border-radius:1rem;border:1px solid var(--default-border);">
                ...
                <div class="modal-body" style="padding:1.5rem;">
                    ...
                    <div class="apy-confirm-note"
                        data-en="You will be redirected to the payment gateway. Do not close the browser tab until payment is complete."
                ...
                <div class="modal-footer border-top" style="padding:1rem 1.5rem;">
                    <button type="button" class="apy-btn-secondary" data-bs-dismiss="modal" data-en="Cancel"
                        data-bm="Batal">Cancel</button>
                    <button type="button" class="apy-btn-pay-confirm" id="apyPayConfirmBtn">
                        <i class="bi bi-send me-1"></i> <span data-en="Confirm &amp; Pay"
                            data-bm="Sahkan &amp; Bayar">Confirm &amp; Pay</span>
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="apy-success-overlay d-none" id="apySuccessOverlay">
        <div class="apy-success-card">
            ...
            <div class="apy-success-title" data-en="Payment Submitted" data-bm="Pembayaran Dihantar">Payment Submitted
            </div>
            ...
            <a href="/public/view_import_permit" class="apy-btn-done">
                <i class="bi bi-arrow-left me-1"></i> <span data-en="Back to Application List"
                    data-bm="Kembali ke Senarai Permohonan">Back to Application List</span>
            </a>
        </div>
    </div>
