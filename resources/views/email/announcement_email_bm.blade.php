                                ...
                                <td style="padding: 40px 50px;">
                                    ...
                                    @if($announcement->valid_from || $announcement->valid_until)
                                        <p style="margin: 24px 0 0 0; font-size: 12px; color: #999999;">
                                            @if($announcement->valid_from && $announcement->valid_until)
                                                Sah: {{ \Carbon\Carbon::parse($announcement->valid_from)->format('d M Y') }} &#8211; {{ \Carbon\Carbon::parse($announcement->valid_until)->format('d M Y') }}
                                            @elseif($announcement->valid_from)
                                                Sah dari: {{ \Carbon\Carbon::parse($announcement->valid_from)->format('d M Y') }}
                                            @elseif($announcement->valid_until)
                                                Sah sehingga: {{ \Carbon\Carbon::parse($announcement->valid_until)->format('d M Y') }}
                                            @endif
                                        </p>
                                    @endif
