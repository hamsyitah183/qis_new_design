...
<head>
    ...
    <title>Consignment Application Summary</title>
    <style>
        @page {
            margin: 1.4cm 1.6cm;
        }
...
<body>
    ...
    <div class="header-title">Consignment Application Summary</div>
    ...
    <table class="app-meta-table">
        <tr>
            ...
            <td class="value">{{ $application->reference_no ?? '-' }}</td>
        ...
        <tr>
            ...
            <td class="label">PTN Number</td>
            <td class="value">{{ $application->ptn_number ?? '-' }}</td>
        </tr>
        <tr>
            <td class="label">Category</td>
            <td class="value">
                {{-- 0 = self-apply, 1 = apply on behalf, per booted() creating() convention elsewhere in the app --}}
        ...
        @if (!empty($application->vehicle_ids))
            <tr>
                <td class="label">Vehicle ID(s)</td>
                {{-- TODO: resolve these against your Vehicle model for the plate/vehicle number
                     instead of the raw IDs, if that lookup is available where this is rendered. --}}
                <td class="value" colspan="3">{{ implode(', ', $application->vehicle_ids) }}</td>
            </tr>
        @endif
    </table>
    ...
    <table class="party-table">
        <tr>
            ...
            <td>
                ...
                {{-- importer_detail is a JSON snapshot taken at submission time; fall back to
                     the live importer() relation if the snapshot is missing a field. --}}
                @php
        ...
        {{-- @if ($application->consignmentPermits->count())
            <tfoot>
                <tr class="total-row">
                    ...
                    <td class="num">RM {{ number_format($application->prices_total ?? 0, 2) }}</td>
                </tr>
            </tfoot>
        @endif --}}
    </table>
