...
function mapActivityLog(json) {
    ...
    const combined = [...RAW_ACTIVITY_LOG, ...qrLogs].sort(
        (a, b) =>
        ...
        try {
            ...
            for (const file of attachments) {
                ...
                try {
                    ...
                    if (!response.ok) {
                        console.warn(
                            `Failed to fetch ${file.name}: ${response.status}`,
                        );
                        continue;
                    }
...
function renderHeaderInfo() {
    ...
    document.getElementById("ipvDownloadBadge").innerHTML =
        `<i class="bi bi-download"></i> ${APPLICATION.downloaded_count}`;
    ...
    document.getElementById("ipvTags").innerHTML = APPLICATION.tags
        .map((tag) => {
...
function renderInfoRow() {
    document.getElementById("ipvAssignedOfficer").textContent =
...
function renderPendingPaymentTable() {
    ...
    document.getElementById("ipvPendingPaymentCount") &&
        (document.getElementById("ipvPendingPaymentCount").textContent =
            pending.length);

    if (!pending.length) {
        tableBody.append(
            `<tr><td colspan="4" class="text-center text-muted">No permits pending payment.</td></tr>`,
        );
        return;
    }
...
function renderPermitDownloadList() {
    ...
    const available = PERMITS.filter(
    ...
    tbody.innerHTML = available
        .map((permit, idx) => {
            const cfg =
                PERMIT_STATUS_CONFIG[permit.status] ||
                PERMIT_STATUS_CONFIG.queued;
            const statusText = cfg[lang] || cfg.en;
            const slug = (permit.permit_number || "").replaceAll("/", "");
            return `
            <tr>
...
function updateSelectedDownloadCount() {
    const count = document.querySelectorAll(
        ".permit-download-checkbox:checked",
    ).length;
    document.getElementById("selectedCount").textContent = count;
    document.getElementById("downloadSelectedPermitsBtn").disabled =
        count === 0;
}
...
document
    .getElementById("selectAllPermits")
    ?.addEventListener("change", function (e) {
        document
            .querySelectorAll(".permit-download-checkbox")
            .forEach((cb) => (cb.checked = e.target.checked));
        updateSelectedDownloadCount();
    });

document
    .getElementById("downloadSelectedPermitsBtn")
    ?.addEventListener("click", function () {
        document
            .querySelectorAll(".permit-download-checkbox:checked")
            .forEach((cb) => {
                window.open(`/permit/generate/${cb.value}`, "_blank");
            });
        const offcanvas = bootstrap.Offcanvas.getInstance(
            document.getElementById("permitListOffcanvas"),
        );
    ...
    $(document).on("click", "#checkoutPage", function (e) {
        ...
        if (!selectedPermits.length) {
            Swal.fire({
                ...
                text:
                    lang === "bm"
                        ? "Pilih permit untuk diteruskan."
                        : "Choose the permit to continue.",
            });
            checkoutLocked = false;
            $btn.prop("disabled", false).text(
                lang === "bm" ? "Pergi Ke Bayaran" : "Go To Checkout",
            );
            return;
        }

        Swal.fire({
            title: lang === "bm" ? "Memuat..." : "Loading...",
            allowOutsideClick: false,
            didOpen: () => Swal.showLoading(),
        });
        ...
        $(".permit-checkbox:checked").each(function () {
            calculatedTotal +=
                parseFloat($(this).attr("data-permit-value")) || 0;
        });
        ...
        $.ajax({
            ...
            error: function () {
                Swal.fire({
                    ...
                    text:
                        lang === "bm"
                            ? "Tidak dapat meneruskan ke bayaran."
...
async function init() {
    ...
    if (!userData) {
        console.warn(
            "init: profile failed to load — role/owner-based UI (permit action buttons, payment banner) will be hidden.",
        );
    }
