...
function loadConsignmentSelection() {
    ...
    const countryCode = application.exporter.country;
    ...
    return fetch(`/get_consignment/${countryCode}`)
        .then((res) => res.json())
        .then((data) => {
...
function loadUses(itemId) {
    ...
    const url =
        itemId === "others"
            ? "/public/consignment_uses"
            : `/public/consignment_uses/${itemId}`;
    fetch(url)
        .then((res) => res.json())
        .then((data) => {
            if (!data.data) return;
            ...
            console.log('load uses', itemId, data)
            $select.select2({
                width: "100%",
...
function itemConsigment() {
    ...
    // ─── When a file is added, add View/Edit/Delete buttons ──
...
// ─── Add custom buttons to Dropzone preview ────────────────
function addPreviewButtons(file) {
    ...
    editBtn.onclick = function (e) {
        ...
        currentItemFile = file;
        showItemFilePreview(file);
        // Optionally open the edit modal/offcanvas – we'll just show preview which has edit input
    };
...
// ─── Show file preview in offcanvas ──────────────────────────
function showItemFilePreview(file) {
    ...
    fileNameSpan.textContent = file.displayName || file.name;
    fileEditInput.value = file.displayName || file.name;

    // Determine if it's an existing file
    const isExisting = file._isExisting === true;
    const fileUrl = file._url || null;

    // Get proper MIME type from file.name if needed (do NOT assign to file.type)
    let mimeType = file.type;
    if (
        !mimeType ||
        mimeType.match(/^(png|jpg|jpeg|gif|webp|pdf|doc|docx)$/i)
    ) {
        mimeType = getMimeType(file.name);
    }

    // ---- Preview ----
    if (isExisting && fileUrl) {
        // Use server URL directly for existing files
        const ext = file.name.split(".").pop().toLowerCase();
        if (["jpg", "jpeg", "png", "gif", "webp"].includes(ext)) {
            previewContainer.innerHTML = `<img src="${fileUrl}" class="img-fluid rounded" alt="${file.displayName || file.name}">`;
        } else if (ext === "pdf") {
            previewContainer.innerHTML = `<iframe src="${fileUrl}" class="w-100" style="height: calc(100vh - 220px); border: none;"></iframe>`;
        } else {
            previewContainer.innerHTML = `
                <div class="text-center">
                    <i class="bi bi-file-earmark-fill fs-1 mb-3"></i>
                    <p>${file.displayName || file.name}</p>
                    <a href="${fileUrl}" target="_blank" class="btn btn-sm btn-primary">Download / View</a>
                </div>
            `;
        }
    } else if (mimeType.startsWith("image/")) {
        // New image file (real File object)
        ...
        previewContainer.dataset.objectUrl = url;
    } else {
        // Generic file: show download option if possible
        if (file.size > 1 || !isExisting) {
            const url = URL.createObjectURL(file);
            previewContainer.innerHTML = `
                <div class="text-center">
                    <i class="bi bi-file-earmark-fill fs-1 mb-3"></i>
                    <p>${file.displayName || file.name}</p>
                    <a href="${url}" target="_blank" download="${file.name}" class="btn btn-sm btn-primary">Download</a>
                </div>
            `;
            previewContainer.dataset.objectUrl = url;
        } else {
    ...
    const fileTypeDisplay = mimeType || file.type || "Unknown";

    fileDetailsDiv.innerHTML = `
        <div class="mb-3">
            <strong>File Name:</strong>
            <div class="text-muted">${file.displayName || file.name}</div>
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <div class="text-muted">${fileTypeDisplay}</div>
        </div>
        ${
            isExisting && fileUrl
                ? `
        <div class="mb-3">
            ...
            <div class="text-muted"><a href="${fileUrl}" target="_blank">${fileUrl}</a></div>
        </div>`
                : ""
        }
    `;

    // Optional: apply translations if available
...
// ─── Save renamed file (for edit button) ──────────────────────
$(document).on("click", "#itemFileSaveBtn", function () {
    ...
    // Update Dropzone preview filename
...
function saveConsignmentAttachment() {
    document.getElementById("saveBtn").addEventListener("click", function (e) {
            ...
            const selectedUnit = measurementUnits.unit.find(
                (unit) =>
                    unit.cate_code.toLowerCase() ===
                        limitMeasurement.toLowerCase() && unit.is_del === false,
            );
            if (selectedUnit) {
        ...
        // ─── Get all files currently in the dropzone ──────────────
        const dropzoneFiles = itemDropzone.getAcceptedFiles();
        ...
        const newItem = {
            ...
            uses: itemUsesValue,
            files: dropzoneFiles, // the complete set (existing + new)
            newFiles: dropzoneFiles.filter((f) => !f._isExisting), // only truly new files
            isCustom: isCustom,
        };
        ...
        if (editingItemId !== null) {
            const index = tempItems.findIndex(
                (obj) => obj.id === editingItemId,
            );
            if (index !== -1) {
                const oldItem = tempItems[index];
                newItem.permit_id = oldItem.permit_id;
                // Replace the whole files array with dropzone files (no duplication)
                tempItems[index] = newItem;
            }
            editingItemId = null;
        } else {
            // New item: all files are new (they are uploaded for the first time)
...
function resetAddItemModal() {
    ...
    // DO NOT reset editingItemId here – it's managed in editItem() and save handler
...
function editItem() {
    $(document).on("click", ".edit-item", function (e) {
        ...
        loadConsignmentSelection()
            ...
            .then(() => {
                // Now uses are loaded, set the value
                if (item.uses) {
                ...
                if (item.purposeValue) {
                    $("#itemPurpose").val(item.purposeValue).trigger("change");
                }
                if (!$("#itemPurpose").val() && item.purpose) {
                    const matchedOption = $("#itemPurpose").find('option').filter(function () {
                        return $(this).text().trim() === item.purpose;
                    });
                    if (matchedOption.length) {
                        $("#itemPurpose").val(matchedOption.val()).trigger("change");
                    }
                }
...
function loadExistingConsignments() {
    ...
    application.consignment_permits.forEach((permit) => {
        ...
        const fileObjects = (permit.attachments || []).map((att) =>
            createExistingFile(att),
        );
        ...
        tempItems.push({
            ...
            purposeValue: detail.purposeValue || null, // store code if exists
            uses: detail.uses,
            isCustom: detail.isCustom || false,
            files: fileObjects, // ✅ now contains File objects
            newFiles: [], // for new uploads during edit
...
$(document).ready(async function () {
    ...
    try {
        ...
        // ─── Reset editingItemId when modal is closed without saving ──
...
export function summarySubmit() {
    ...
    if (document.getElementById("expName"))
        document.getElementById("expName").value = exporter.name;
    document.getElementById("sexpfonno").textContent = exporter.phone_no;
    if (document.getElementById("expfonno"))
        document.getElementById("expfonno").value = exporter.phone_no;
    document.getElementById("sexpAddress").textContent = exporter.address;
    if (document.getElementById("expAddress"))
        document.getElementById("expAddress").value = exporter.address;
    document.getElementById("sexpCountry").textContent = exporter.country;
    if (document.getElementById("expCountry"))
        document.getElementById("expCountry").value = exporter.country;
