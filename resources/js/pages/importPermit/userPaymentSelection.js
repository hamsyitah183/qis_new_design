...
const PERMITS = [
    {
        ...
        decision: 'approved', selected: true,
        ...
        permitActivity: [
            { stage: 'doc_verification', title: 'Documents Verified',    description: 'Verified by clerk Nurul Aisyah.',          time: '13 May 2025, 8:50 AM' },
            { stage: 'permit_approved',  title: 'Permit Approved',       description: 'Approved by officer Ahmad Zulkifli.',       time: '14 May 2025, 11:05 AM' },
        ],
    },
    {
        ...
        decision: 'rejected', selected: false,
        rejectionReason: 'Annual import quota for this species has been exhausted.',
        ...
        permitActivity: [
            { stage: 'doc_verification', title: 'Documents Verified', description: 'Verified by clerk Nurul Aisyah.',                                         time: '13 May 2025, 8:52 AM' },
            { stage: 'permit_rejected',  title: 'Permit Rejected',    description: 'Rejected — annual import quota exhausted.',                               time: '14 May 2025, 11:20 AM' },
        ],
    },
    {
        ...
        permitActivity: [
            { stage: 'doc_verification', title: 'Documents Verified', description: 'Verified by clerk Nurul Aisyah.',    time: '13 May 2025, 8:54 AM' },
            { stage: 'permit_approved',  title: 'Permit Approved',    description: 'Approved by officer Ahmad Zulkifli.', time: '14 May 2025, 3:10 PM'  },
        ],
    },
    {
        ...
        decision: 'approved', selected: false,
        rejectionReason: null,
        remark: 'Approved by officer Ahmad Zulkifli.',
        attachments: [
        ...
        permitActivity: [
            { stage: 'doc_verification', title: 'Documents Verified', description: 'Verified by clerk Nurul Aisyah.',    time: '13 May 2025, 8:55 AM' },
            { stage: 'permit_approved',  title: 'Permit Approved',    description: 'Approved by officer Ahmad Zulkifli.', time: '14 May 2025, 3:15 PM'  },
        ],
    },
];
...
const ACTIVITY_LOG = [
    ...
    { stage: 'doc_verification', title: 'Stage: Submitted → Document Verification',      description: 'Assigned to clerk Nurul Aisyah.',                        time: '12 May 2025, 10:15 AM' },
    { stage: 'doc_verification', title: 'Documents Verified by Clerk',                   description: 'Forwarded to Technical Review.',                          time: '13 May 2025, 9:00 AM'  },
    ...
    { stage: 'permit_approved',  title: 'Permit PMT-1201 Approved',                      description: 'Approved by Ahmad Zulkifli.',                             time: '14 May 2025, 11:05 AM' },
    { stage: 'permit_rejected',  title: 'Permit PMT-1202 Rejected',                      description: 'Import quota exhausted.',                                 time: '14 May 2025, 11:20 AM' },
    { stage: 'permit_approved',  title: 'Permit PMT-1203 Approved',                      description: 'Approved by Ahmad Zulkifli.',                             time: '14 May 2025, 3:10 PM'  },
    { stage: 'permit_approved',  title: 'Permit PMT-1204 Approved',                      description: 'Approved by Ahmad Zulkifli.',                             time: '14 May 2025, 3:15 PM'  },
    { stage: 'awaiting_payment', title: 'Stage: Technical Review → Awaiting Payment',    description: 'Invoice generated. Awaiting applicant payment.',          time: '14 May 2025, 3:20 PM'  },
];
...
function paintAttachmentList(containerEl, files, visibleCount) {
    ...
    if (remaining > 0) html += `<div class="ipv-attach-more" data-list-id="${listId}">+${remaining}</div>`;
...
document.addEventListener('click', e => {
    ...
    if (chip) { openAttachmentViewer(chip.dataset.listId, parseInt(chip.dataset.index, 10)); return; }
...
function openAttachmentViewer(listId, index) {
    ...
    currentListId = listId; currentIndex = index;
...
function renderViewer(file) {
    ...
    if (!path) { container.innerHTML = '<div class="text-muted"><i class="bi bi-file-earmark-fill fs-1"></i><br>No file available</div>'; return; }
    if (mime.startsWith('image/'))    container.innerHTML = `<img src="${escapeHtml(path)}" style="max-width:100%;max-height:70vh;">`;
    else if (mime === 'application/pdf') container.innerHTML = `<iframe src="${escapeHtml(path)}" style="width:100%;height:70vh;border:none;"></iframe>`;
    else container.innerHTML = `<div class="text-center"><i class="bi bi-file-earmark-fill fs-1 d-block mb-3" style="color:var(--gray-5);"></i><p class="text-muted">Preview not available.</p></div>`;
}
...
function openPermitDetail(permitNumber) {
    ...
    const decisionCfg = PERMIT_DECISION_CONFIG[permit.decision] || { en: 'Pending', color: 'secondary' };
    ...
    document.getElementById('pdActivityTimeline').innerHTML =
        (permit.permitActivity || []).map(entry => {
...
function renderPermitAccordion() {
    ...
    el.innerHTML = PERMITS.map(permit => {
        ...
        return `
            <div class="ipv-permit-item ${permit.selected ? 'is-open ps-item-selected' : ''} ${isRejected ? 'ps-item-rejected' : ''}"
                 data-permit="${escapeHtml(permit.permit_number)}">

                <div class="ipv-permit-header">

                    <!-- Checkbox — only active for approved permits -->
                    <label class="ps-check-label" title="${isRejected ? 'Rejected — cannot be selected' : 'Toggle selection'}"
                    ...
                    <div class="ipv-permit-icon ${isRejected ? 'ps-icon-rejected' : ''}">
                    ...
                    ${permitStatusBadge(permit)}

                    <div class="ipv-permit-value">RM ${money(permit.value)}</div>

                    ${isApproved ? `
                        <div class="ps-fee-chip">
                            <i class="bi bi-tag"></i> RM ${ITEM_PRICE}
                        </div>
                    ` : `
                        <div class="ps-fee-chip is-na">—</div>
                    `}
    ...
    // Delegated click handler for the whole accordion
    let _accordionInit = false;
    if (!_accordionInit) {
        _accordionInit = true;
        document.getElementById('psPermitAccordion').addEventListener('click', e => {
            // View detail button
            const viewBtn = e.target.closest('.ipv-view-detail-btn');
            if (viewBtn) { e.stopPropagation(); openPermitDetail(viewBtn.dataset.permitNumber); return; }

            // Checkbox change is handled via 'change' event below
            if (e.target.closest('.ps-check-label')) return;

            // Accordion toggle
            const header = e.target.closest('.ipv-permit-header');
            if (header) header.closest('.ipv-permit-item')?.classList.toggle('is-open');
        });

        document.getElementById('psPermitAccordion').addEventListener('change', e => {
...
function updateSummary() {
    ...
    document.getElementById('psCountRejected').textContent = rejectedPermits.length;
    ...
    document.getElementById('psFeeTotalLabel').textContent = `RM ${money(total)}`;
